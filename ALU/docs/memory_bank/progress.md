...
**VERIFICATION COMPLETE** — 全実装・テストベンチ強化済み。シミュレーション実行待ち。
...
- [ ] **Icarus Verilog インストール → シミュレーション実行**
  - `https://bleyer.org/icarus/` からWindows版をダウンロード
  - または Gowin EDA のシミュレーション機能を使用
- [ ] シミュレーション結果の解析（ログ・VCD波形）
- [ ] Gowin EDA で論理合成・配置配線
- [ ] FPGA実機書き込み・動作確認
- [ ] レポート作成（ALU_SPECIFICATION.md の内容を流用可能）
